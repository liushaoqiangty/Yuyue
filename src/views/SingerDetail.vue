 <template>
  <div class="recomDetail" ref="detail">
    <div>
      <div class="header">
      ...
      <div class="bg-image" v-lazy:background-image="singerInfo && singerInfo.picUrl">
        <div class="text">
          <h2 class="list-title">{{ singerInfo && singerInfo.name }}</h2>
          <p class="update" v-if="this.$route.name!='singer'">最近更新:{{ list.trackUpdateTime}}</p>
          <p class="play-count" v-if="this.$route.name==='singer'">
            <span class="fa fa-user-o"></span>
            {{ singerInfo.accountId | countFilter }}人已收藏
          </p>
        </div>
      </div>
      <div class="list-content">
        <div class="content-header">
          ...
          <span>播放全部</span>
          <span class="small">(共{{ list && list.length }}首)</span>
        </div>
        <div class="list-all">
          <ul>
            <li
